---
name: release
description: Cut a cui-test-keycloak-integration release — bump .github/project.yml version, open and merge the release PR, wait for the automated Release workflow, verify the release landed, then reformat the generated GitHub release notes
user-invocable: true
...
Cuts a new cui-test-keycloak-integration release end-to-end: determine the version, open the version-bump PR
...
```yaml
consumers:
  - cuioss-parent-pom:version.cui.test.keycloak.integration
  - nifi-extensions:version.cui.test.keycloak.integration
```
...
- `release.current-version` (currently `1.3.0`) — the **last released** version.
- `release.next-version` (currently `1.4.0-SNAPSHOT`) — what `pom.xml` carries between releases.

**Default rule:** the release version is `next-version` with `-SNAPSHOT` stripped —
`1.4.0-SNAPSHOT` → **`1.4.0`**. The new `next-version` is the next minor plus `-SNAPSHOT`
(`1.5.0-SNAPSHOT`).

This project carries a **three-segment** `next-version`, so the floor moves with every
release. A patch release (`1.4.0` → bump the last segment) is possible but is not the
established shape here — mirror the existing scheme rather than inventing one.

### Step 2 — Check for open PRs

```bash
gh pr list --repo cuioss/cui-test-keycloak-integration --state open --json number,title,isDraft
```
- **No open PRs** → proceed.
...
```bash
git checkout -b chore/release_1.4.0
```
...
if ! gh label list --repo cuioss/cui-test-keycloak-integration --search skip-bot-review --json name --jq '.[].name' \
     | grep -qx skip-bot-review; then
  gh label create skip-bot-review --repo cuioss/cui-test-keycloak-integration \
    --description "Skip automated bot review" --color ededed
fi
gh pr create --repo cuioss/cui-test-keycloak-integration --base main \
...
```bash
gh pr checks <pr#> --repo cuioss/cui-test-keycloak-integration --watch
```
...
- Every review comment MUST get a reply and MUST be resolved — fix it and say so, or explain
  ...
  ```bash
  gh api --paginate repos/cuioss/cui-test-keycloak-integration/pulls/<pr#>/comments
  ```
- **Unresolved review threads block the merge.** A PR can show every check green and still
  ...
  ```bash
  gh api graphql --paginate -f query='query($endCursor:String){repository(owner:"cuioss",name:"cui-test-keycloak-integration"){pullRequest(number:<pr#>){reviewThreads(first:100,after:$endCursor){pageInfo{hasNextPage endCursor} nodes{id isResolved}}}}}'
...
```bash
gh pr merge <pr#> --repo cuioss/cui-test-keycloak-integration --squash --delete-branch
```
...
```bash
gh run list --repo cuioss/cui-test-keycloak-integration --workflow "Release" --limit 3 \
  --json status,conclusion,displayTitle,databaseId
gh run watch <databaseId> --repo cuioss/cui-test-keycloak-integration
```

### Step 11 — Verify the release landed

```bash
gh release view <version> --repo cuioss/cui-test-keycloak-integration --json tagName,name,isDraft,createdAt
git fetch --tags && git tag --list <version>
curl -sfI https://repo1.maven.org/maven2/de/cuioss/test/cui-test-keycloak-integration/<version>/cui-test-keycloak-integration-<version>.pom \
...
```bash
mkdir -p .plan/temp
gh release view <version> --repo cuioss/cui-test-keycloak-integration --json body --jq .body \
  > .plan/temp/release-<version>-orig.md
# ...build the reformatted body in .plan/temp/release-<version>.md...
gh release edit <version> --repo cuioss/cui-test-keycloak-integration --notes-file .plan/temp/release-<version>.md
```
